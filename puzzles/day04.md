...
Maybe it wants to play [__bingo__](https://en.wikipedia.org/wiki/Bingo_(American_version))?
...
At this point, the third board __wins__ because it has at least one complete row or column of marked numbers (in this case, the entire top row is marked: 14 21 17 24 4).

The __score__ of the winning board can now be calculated. Start by finding the __sum of all unmarked numbers__ on that board; in this case, the sum is `188`. Then, multiply that sum by __the number that was just called__ when the board won, `24`, to get the final score, `188 * 24 = 4512`.
